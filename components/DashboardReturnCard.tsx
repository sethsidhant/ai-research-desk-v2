...
function OverviewBriefModal({
  ...
  useEffect(() => {
    const cacheKey = `oasis_overview_brief_${type}`
  ...
  async function fetchBrief() {
    ...
    try {
      ...
      try {
        sessionStorage.setItem(`oasis_overview_brief_${type}`, JSON.stringify({ brief: data, date: new Date().toISOString().slice(0, 10) }))
      } catch {}
    } catch {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="px-5 pb-4 text-[10px] text-center" style={{ color: 'var(--artha-text-faint)' }}>
          {cached ? 'Cached today · ' : ''}Generated by Oasis AI · Haiku · Not investment advice
        </div>
      </div>
    </div>
  )
